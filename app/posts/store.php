...
if (isset($_FILES['post'])) {
    ...
    if (!in_array($post['type'], ['image/jpeg', 'image/png'])) {
        ...
        $date = date('now');
        $caption = trim(filter_var($post['caption'],FILTER_SANITIZE_STRING));

        $queryFetchAvatars = 'SELECT * FROM posts WHERE user_id = :user_id';
        ...
        if($posts===false) {
            ...
            move_uploaded_file($post['tmp_name'], __DIR__.'/app/database/posts/'.$postPath);
